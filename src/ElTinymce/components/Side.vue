  ...
  <div class="el-tinymce-side" v-if="sideShow">
    ...
    <div class="el-tinymce-assets-list">
      <template v-for="(item,index) in list" v-if="itemShow[item.type]">
        <div :class="`el-tinymce-assets-item el-tinymce-assets-item-${item.type}`"
             @click="dialogShow = item.type"><i class="el-tinymce-assets-item-icon"></i>{{item.title}}
        </div>
        <el-dialog :key="index" :title="item.title" :visible="dialogShow===item.type" @close="dialogClose(index)"
                   class="el-tinymce-dialog">
          <el-tabs v-model="item.dialog.activeName">
            <el-tab-pane :label="tab.title" :name="item.type+tabIndex" :key="tabIndex"
              ...
              <el-form :model="tab.formData" :rules="tab.formRules" :ref="tab.formName" label-width="0">
                ...
                <el-form-item class="el-tinymce-dialog-btn">
                  <el-button @click="reset($refs[tab.formName][0])">{{i18n.btn.reset}}</el-button>
                  <el-button type="primary" @click="submit($refs[tab.formName][0],tab.formData,item.dialog.template)">
                    {{i18n.btn.submit}}
                  </el-button>
                </el-form-item>
              </el-form>
...
<script>
  ...
  import { Dialog, Tabs, TabPane, Form, FormItem, Input, Button } from 'element-ui'
